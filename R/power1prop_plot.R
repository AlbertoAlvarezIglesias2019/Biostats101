#' @title Plot Power Curves for One-Sample proportion
#'
#' @description
...
#'     \item \code{p2}: Proportion under the alternative.
...
#'     \item \code{p1}: Proportion under the null
#'   }
#'
#' @return
...
#' proportion (\code{p2}) and the resulting "Actual Power" (\code{ActualP}).
#'
#' @examples
#' \dontrun{
...
#' # The data must contain columns: X, Y, Ss, alt_ttest, alpha, p1, p2, ActualP.
#'
...
#'                                   alt_ttest = "greater")
#' power1prop_plot(pocu = results_power$data_plot)
#'
...
power1prop_plot <- function(pocu) { 
  ...
  power_curve <- ggplot(pocu, aes(x = X, y = Y,colour = as.factor(Ss),linetype = as.factor(Ss))) +
    ...
    xlab(bquote(paste("Comparison ",p[0]," = ",.(p1_value),sep=""))) +
    ylab("Power")+
