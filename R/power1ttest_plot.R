#' @title Plot Power Curves for One-Sample t-Test
#'
#' @description
...
power1ttest_plot <- function(pocu) { 
  ...
  datpo <- pocu %>% dplyr::select(Diff,Ss,ActualP) %>% dplyr::distinct()
  datpo$ActualP <- as.numeric(datpo$ActualP)
  
  if (pocu$alt_ttest[1]=="two.sided") {
    temp <- datpo %>% dplyr::mutate(Diff = -Diff)
  ...
  power_curve <- ggplot(pocu, aes(x = X, y = Y,colour = as.factor(Ss),linetype = as.factor(Ss))) +
    # Add the power line
    geom_line() +
    geom_point(data = datpo,aes(x=as.numeric(Diff),y=ActualP),size=3) +
    scale_y_continuous(
    ...
    labs(
      title = paste("Power Curve for One-Sample T-Test"),
      color = "Sample Size",
