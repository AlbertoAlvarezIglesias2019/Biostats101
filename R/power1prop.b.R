...
power1propClass <- if (requireNamespace('jmvcore', quietly=TRUE)) R6::R6Class(
    "power1propClass",
    inherit = power1propBase,
    private = list(
        ...
        if (any(cond1,cond2)) {
          ...
          if (cond1) {
            self$results$warning_message_1$setContent(
              paste(
                ...
                "Warning: p<sub>a</sub> must be greater than p<sub>0</sub> for a <i>greater than</i> alternative hypothesis.",
                "</p>"
              )
            )
          ...
          if (cond2) {
            self$results$warning_message_1$setContent(
              paste(
                "<p style='color: #D35400; font-size: 1.5em; font-weight: bold;'>",
                "Warning: p<sub>a</sub> must be less than p<sub>0</sub> for a <i>less than</i> alternative hypothesis.",
                "</p>"
              )
            )
            self$results$warning_message_1$setVisible(TRUE)
