...
#' results_n <- power2ttest_html(
#'   solvefor = "n",
#'   delta = 5, # Three different effect sizes
#'   sd = 20,
#'   sig.level = 0.05,
...
#' # power2ttest_plot(results_n$data_plot) # Assuming power1ttest_plot is available
#'
...
power2ttest_html <- function(solvefor="n",n=NULL, delta=NULL, sd, sig.level, power=NULL, alt_ttest = "two.sided", font_size=18,nd=1) { 
  ...
  temp <- lapply(1:dim(pointer)[1],function(i) {
    ...
    if (solvefor=="n") {
      fittp <- stats::power.t.test(n=ceiling(fit$n),
                                   ...
                                   power=NULL,type ="two.sample", alternative = alternative,
                                   strict = TRUE)
  ...
  fn1 <- "Two-Sample t Test"
  fn2 <- dplyr::case_when(
    alt_ttest == "greater" ~ paste("Testing &mu;<sub>1</sub> &le; &mu;<sub>2</sub> (versus &mu;<sub>1</sub> &gt; &mu;<sub>2</sub>)", sep = ""),
    alt_ttest == "less" ~ paste("Testing &mu;<sub>1</sub> &ge; &mu;<sub>2</sub> (versus &mu;<sub>1</sub> &lt; &mu;<sub>2</sub>)", sep = ""),
    alt_ttest == "two.sided" ~ paste("Testing &mu;<sub>1</sub> = &mu;<sub>2</sub> (versus &mu;<sub>1</sub> &ne; &mu;<sub>2</sub>)", sep = "")
  )
  fn3 <- dplyr::case_when(
    alt_ttest == "greater" ~ paste("Calculating power for &mu;<sub>1</sub> = &mu;<sub>2</sub> + difference", sep = ""),
    alt_ttest == "less" ~ paste("Calculating power for &mu;<sub>1</sub> = &mu;<sub>2</sub> - difference", sep = ""),
    alt_ttest == "two.sided" ~ paste("Calculating power for &mu;<sub>1</sub> = &mu;<sub>2</sub> &plusmn; difference", sep = "")
  )
  ...
  footnotes_html <- c(fn0,fn1, fn2, fn3, fn4, fn5)
